...
def test_dispute_defaults():
    ...
    # mrp_abuse is intentionally excluded from autonomous by default (OCR risk).
    assert "mrp_abuse" not in settings.dispute_autonomous_categories
    assert "expiry" in settings.dispute_autonomous_categories
